...
#[derive(Clone)]
pub(crate) struct PaneStates {
  pub(crate) left: PaneState,
  pub(crate) right: PaneState,
}
...
impl WorkspaceModel {
  ...
  fn next_terminal_session_id(&mut self) -> String {
  ...
  pub(crate) fn close_window_and_collect_terminal_sessions(
    ...
    for tab_id in &window.tab_order {
      if let Some(tab) = self.tabs.remove(tab_id) {
        session_ids.push(tab.pane_states.left.terminal_session_id);
        session_ids.push(tab.pane_states.right.terminal_session_id);
      }
    }
...
mod tests {
  ...
  fn close_window_if_empty_or_fix_active_repairs_invalid_active_tab() {
    ...
    let window = model.windows.get(&window_id).expect("window should still exist");
