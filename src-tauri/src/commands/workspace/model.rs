use super::types::{
    ...
    DEFAULT_RIGHT_PANEL_TYPE, DEFAULT_SPLIT_RATIO,
};
...
impl WorkspaceModel {
    ...
    pub(crate) fn close_window_if_empty_or_fix_active(
        &mut self,
        window_id: &str,
    ) -> Option<String> {
...
mod tests {
    ...
    fn ensure_window_bootstrap_creates_default_tab() {
        ...
        assert_eq!(
            tab.active_pane_id,
            find_first_layout_pane_id(&tab.layout)
                .expect("layout should contain at least one pane"),
        );
