...
pub fn workspace_move_tab(
  ...
  let (snapshot, maybe_closed_label, should_exit) = {
    ...
    let source_contains_tab = model
      ...
      .map(|window| window.tab_order.iter().any(|tab_id| tab_id == &payload.tab_id))
      .unwrap_or(false);
    ...
    (snapshot, maybe_closed_label, should_exit)
  };
...
pub fn workspace_close_tab(
  ...
  let (snapshot, terminal_session_ids, maybe_closed_label, should_exit) = {
    ...
    let terminal_session_ids = vec![
      tab.pane_states.left.terminal_session_id,
      tab.pane_states.right.terminal_session_id,
    ];
...
pub fn workspace_close_window(
  ...
  let (snapshot, terminal_session_ids, maybe_closed_label, should_exit) = {
    ...
    let should_exit = model.windows.is_empty();
    let snapshot = model.snapshot();
    (snapshot, session_ids, Some(closed_label), should_exit)
  };
...
pub fn handle_runtime_window_destroyed(
  ...
  let (maybe_snapshot, terminal_session_ids, should_exit) = {
    let mut model = match state.inner.lock() {
      Ok(model) => model,
      Err(_) => return,
    };

    let Some(window_id) = model.window_ids_by_label.get(window_label).cloned() else {
      return;
    };
    let Some((_, session_ids)) = model.close_window_and_collect_terminal_sessions(&window_id)
    else {
      return;
    };
